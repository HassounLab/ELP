...
class EMEmbedding:
    def __init__(self, **params):
        ...
        for k, v in default_params.items():
            if k not in params:
                params[k] = v
        assert params['use_node_embed'] or params['use_fgpt']

        if "hidden_sizes" not in params['nn']:
            params['nn']["hidden_sizes"] = [max(4, params['embed_size'] // 2),
                                              max(2, params['embed_size'] // 4)]
        if "random_seed" not in params['nn']:
            params['nn']["random_seed"] = params['random_seed']
