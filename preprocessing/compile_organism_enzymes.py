...
def compile_organism_edges():
    ...
    df_ee = get_df_enzymes_edges()
    print('df_enzymes_edges')
    print(df_ee)
    def combine_edges_for_enzymes(ecs):
        edges = set([])
        for ec in ecs.split():
            if '-' in ec:
                ec_sup = '.'.join(ec.split('.')[:-1])
                #if sum(df_ee['ec_sup'] == ec_sup) == 0:
                #    print('EC sup', ec_sup, 'DNE')
                
                for _, e_edges in df_ee[df_ee['ec_sup'] == ec_sup]['edges'].iteritems():
                    edges |= e_edges
            elif ec not in df_ee.index:
                #print('EC', ec, 'DNE')
                continue
            else:
                edges |= df_ee.loc[ec, 'edges']
        return edges
            
    df['edges'] = df['enzymes'].apply(combine_edges_for_enzymes)
    df['num_edges'] = df['edges'].apply(len)
    print('df_organism_edges')
    print(df)
    dfpath = '%s/kegg/organism_edges.csv' % os.environ['DATAPATH']
    df.to_csv(dfpath, index=False)
    print('saved to', dfpath)
    print('Number of edges per phylum')
    phylum_to_edges = {}
    for p, df_p in df.groupby('phylum'):
        edges = set([])
        for _, p_edges in df_p['edges'].iteritems():
            edges |= p_edges
        phylum_to_edges[p] = edges
        print(p, len(edges))
...
def get_df_enzymes_edges():
    ...
    with open(reaction_lst) as f:
        ...
        for line in f:
            ...
            for r in reactants:
                for p in products:
                    if r != p and G.has_edge(r, p):
                        c1, c2 = min(r, p), max(r, p)
                        rn_to_edges[rn].add((c1, c2))
    def combine_edges_for_reactions(rns):
        edges = set([])
        for rn in rns.split():
            edges |= rn_to_edges[rn]
        return edges
        
    df['edges'] = df['reactions'].apply(combine_edges_for_reactions)
    df['num_edges'] = df['edges'].apply(len)
    df = df.set_index('ec')
    return df
